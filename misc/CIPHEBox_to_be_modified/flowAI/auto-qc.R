...
#' @export
source("flowAI/anomaly-detection.R")
source("flowAI/cffilter.R")
source("flowAI/data.R")
source("flowAI/flow-rate.R")
source("flowAI/flow-set.R")
source("flowAI/margins.R")
source("flowAI/signal.R")
source("flowAI/support-qc.R")

flow_auto_qc.CIPHE <- function(fcsfiles, remove_from = "all", output = 1,
  ...
  if( output == 1 || output == 2){
      if(length(out) == 1){ 
        res <- list(flow.frame=out[[1]],data=list(FlowRateQC,FlowSignalQC,FlowMarginQC))
        return(res)
      }else{
